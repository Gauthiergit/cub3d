...
/*   Updated: 2024/05/07 10:46:49 by gdetourn         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_move_forward(t_data *data)
{
	...
	x = roundf(data->player.px_x + sin(data->player.angle) * data->move_speed);
	y = (int)data->player.px_y;
	if (data->scene[(int)floor(y / SQUARE_SIZE)][(int)floor(x / SQUARE_SIZE)] != '1')
	{
		data->player.px_x = x;
	...
	y = roundf(data->player.px_y + cos(data->player.angle) * data->move_speed);
	x = (int)data->player.px_x;
	if (data->scene[(int)floor(y / SQUARE_SIZE)][(int)floor(x / SQUARE_SIZE)] != '1')
	{
		data->player.px_y = y;
